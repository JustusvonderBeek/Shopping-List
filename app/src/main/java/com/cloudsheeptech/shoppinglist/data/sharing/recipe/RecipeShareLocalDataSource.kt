...
class RecipeShareLocalDataSource @Inject constructor(
    ...
    suspend fun create(recipeId: Long, createdBy: Long, sharedWith: Long): RecipeShare {
        var createdSharedWith: RecipeShare
        withContext(Dispatchers.IO) {
            val user = userRepo.read() ?: throw IllegalStateException("user not initialized")
            val recipe = recipeRepo.read(recipeId, createdBy)
    ...
    fun readLive(recipeId: Long, createdBy: Long): LiveData<List<RecipeShare>> {
        return shareDao.getLive(recipeId, createdBy)
    }
