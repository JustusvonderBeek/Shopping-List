...
import com.cloudsheeptech.shoppinglist.network.token.ITokenProvider
...
class ShoppingListNetworkHandler
@Inject
constructor(
    ...
    private val networkClient =
        HttpClient(OkHttp) {
            ...
            install(Auth) {
                bearer {
                    // Only for stored tokens
                    loadTokens {
                        tokenProvider.loadToken()
                    }
                    // Called when loadTokesn doesn't return anything
                    refreshTokens {
                        tokenProvider.refreshToken()
                    }
                    sendWithoutRequest { request ->
                        request.url.host == "127.0.0.1" || request.url.host == "shop.cloudsheeptech.com"
                    }
                }
            }
            install(HttpTimeout) {
    ...
    override suspend fun get(
        ...
        for (i in 0..<retryCount) {
            success = getImpl(requestUrlPath, responseHandler)
            if (success) {
                break
            }
        }
    ...
    private suspend fun post_internal(
        ...
        withContext(Dispatchers.IO) {
            try {
                val finalRequestUrl = "${UrlProviderEnum.BASE_URL.url}$requestUrlPath"
                val response =
                    networkClient.post(finalRequestUrl) {
                        if (data != null) {
                            setBody(data)
                        }
                    }
    ...
    private suspend fun put_internal(
        ...
        withContext(Dispatchers.IO) {
            try {
                val finalRequestUrl = "${UrlProviderEnum.BASE_URL.url}$requestUrlPath"
                val response =
                    networkClient.put(finalRequestUrl) {
                        setBody(data)
                    }
                success = responseHandler.handle(response)
            } catch (ex: ConnectException) {
    ...
    private suspend fun patch_internal(
        ...
        withContext(Dispatchers.IO) {
            try {
                val finalRequestUrl = "${UrlProviderEnum.BASE_URL.url}$requestUrlPath"
                val response =
                    networkClient.patch(finalRequestUrl) {
                        setBody(data)
                    }
                success = responseHandler.handle(response)
            } catch (ex: ConnectException) {
