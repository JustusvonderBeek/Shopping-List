...
class ShoppingListOfflineTest {
    ...
    fun testCreateList() = runTest {
        ...
        val listWithItems = ApiShoppingList(3L, "list with items", ListCreator(1440L, "remote creator"), OffsetDateTime.now(), OffsetDateTime.now(), mutableListOf())
        for (num in 1..3) {
...
//        Log.d("ShoppingListOfflineTest", "RetrievedList: $retrievedList")
    ...
    fun testUpdateList() = runTest {
        ...
        val updatedRetrievedList = localDataSource.read(listWithItems.listId, listWithItems.createdBy.onlineId)
        ...
        localDataSource.update(listWithItems)
        val updatedItemsRetrievedList = localDataSource.read(listWithItems.listId, listWithItems.createdBy.onlineId)
    ...
    fun testDeleteList() = runTest {
        ...
        val database = ShoppingListDatabase.getInstance(application)
        val itemToList = database.mappingDao()
        val mappings = itemToList.getMappingsForList(listWithItems.listId, listWithItems.createdBy.onlineId)
