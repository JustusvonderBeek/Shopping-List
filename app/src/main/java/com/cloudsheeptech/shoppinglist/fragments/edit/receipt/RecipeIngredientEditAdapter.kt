...
class RecipeIngredientEditAdapter(
...
) : ListAdapter<ApiIngredient, RecipeIngredientEditAdapter.ReceiptItemViewHolder>(ItemDiffCallback()) {
    ...
    class ReceiptIngredientEditClickListener(
        val clickListener: (itemId: Long, quantity: Int) -> Unit,
    ) {
        fun onClick(
            ingredient: ApiIngredient,
            quantity: Int,
        ) = clickListener(ingredient.id, quantity)
    }
