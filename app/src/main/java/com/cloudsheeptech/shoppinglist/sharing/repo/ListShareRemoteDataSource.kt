...
@Singleton
class ListShareRemoteDataSource
    @Inject
    constructor(
        ...
        suspend fun read(listId: Long): List<Long> = throw NotImplementedError("this should never be called from online")
        ...
        suspend fun delete(
            ...
            withContext(Dispatchers.IO) {
                networking.DELETE(
                    "${UrlProviderEnum.SHOPPING_LIST_SHARE.url}/$listId?createdBy=$createdBy&sharedWith=$sharedWith",
                ) { response ->
                    if (response.status != HttpStatusCode.OK) {
