...
//    private val createdBy : Long
        ...
        private fun combineMappingAndItemToItemWithQuantity(
            mapping: ListMapping,
            dbItem: DbItem,
        ): AppItem =
            AppItem(
        ...
        fun toggleItem(itemId: Long) {
            ...
            localCoroutine.launch {
                val migratedToNewId = shoppingListRepository.toggleItem(shoppingListId, createdBy, itemId)
                withContext(Dispatchers.Main) {
        ...
        fun increaseItemCount(
            ...
            localCoroutine.launch {
                val migratedToNewId = shoppingListRepository.updateItemCount(shoppingListId, createdBy, itemId.toLong(), quantity)
                if (migratedToNewId) {
                    withContext(Dispatchers.Main) {
                        navigateUp()
                    }
        ...
        fun decreaseItemCount(itemId: Int) {
            localCoroutine.launch {
                val migratedToNewId = shoppingListRepository.updateItemCount(shoppingListId, createdBy, itemId.toLong(), -1L)
                if (migratedToNewId) {
                    withContext(Dispatchers.Main) {
                        navigateUp()
                    }
